-- Script SQL corregido para agregar subcategorías
...
-- 5. Redefinir la vista expenses_with_installments para incluir subcategorías
CREATE OR REPLACE VIEW expenses_with_installments AS
SELECT 
    e.id,
    e.user_id,
    e.card_id,
...
-- 6. Configurar RLS (Row Level Security) para subcategories
...
-- Política para permitir solo a administradores crear subcategorías
CREATE POLICY "Allow admins to create subcategories" ON subcategories
...
-- Política para permitir solo a administradores actualizar subcategorías
CREATE POLICY "Allow admins to update subcategories" ON subcategories
    FOR UPDATE USING (
        EXISTS (
            SELECT 1 FROM users 
            WHERE users.id = auth.uid() 
...
-- Política para permitir solo a administradores eliminar subcategorías
CREATE POLICY "Allow admins to delete subcategories" ON subcategories
    FOR DELETE USING (
        EXISTS (
            SELECT 1 FROM users 
            WHERE users.id = auth.uid() 
...
-- 7. Crear función para obtener categorías con sus subcategorías
...
-- 8. Crear función para obtener subcategorías por categoría
...
-- 9. Crear función para obtener una subcategoría por ID
